...
function ccgallery_admin_scripts() {
    wp_enqueue_style( 'ccgallery_upload_css', ccgallery_css_path.'/fileuploader.css', null, null );
    wp_enqueue_style( 'ccgallery_admin_css', ccgallery_css_path.'/admin.css', array('ccgallery_upload_css'), null );

    wp_enqueue_script( 'jquery' );
    wp_enqueue_script( 'jquery-ui-sortable' );
    wp_enqueue_script( 'ccgallery_upload_js', ccgallery_js_path.'/fileuploader.js', null, null );
    wp_enqueue_script( 'ccgallery_admin_js', ccgallery_js_path.'/admin.js', array('jquery', 'jquery-ui-sortable', 'ccgallery_upload_js'), null );
...
function ccgallery_shortcode( $attr ) {
    ...
    wp_enqueue_script( 'ccgallery_touch_punch', ccgallery_js_path.'/jquery.ui.touch-punch.min.js', array('jquery', 'jquery-ui-slider'), null, true );
    wp_enqueue_script( 'ccgallery_mousewheel', ccgallery_js_path.'/jquery.mousewheel.min.js', array('jquery'), null, true );
    wp_enqueue_script( 'ccgallery_quicksand', ccgallery_js_path.'/jquery.quicksand.mod.min.js', array('jquery'), null, true );
    wp_enqueue_script( 'ccgallery_innershiv', ccgallery_js_path.'/innershiv.min.js', null, null, true );
    wp_enqueue_script( 'ccgallery_mediaelement_js', ccgallery_path.'/mediaelement/mediaelement-and-player.min.js', array('jquery'), null, true );
    wp_enqueue_script( 'modernizr', ccgallery_js_path.'/modernizr.js', null, null, true );
    wp_enqueue_script( 'ccgallery_js', ccgallery_js_path.'/jquery.ccgallery.min.js', array('jquery', 'jquery-ui-slider', 'ccgallery_touch_punch',  'ccgallery_mousewheel', 'ccgallery_quicksand', 'ccgallery_innershiv', 'ccgallery_mediaelement_js', 'modernizr'), null, true );
    ...
    if( (isset($gallery_option['hideCoverflow']) && $gallery_option['hideCoverflow'] === 'true') || (isset($gallery_option['hideThumbnails']) && $gallery_option['hideThumbnails'] === 'true') ) {
    ...
    $hideCoverflow = isset($gallery_option['hideCoverflow']) && $gallery_option['hideCoverflow'] === 'true' ? ' style="display: none"' : '';
    $hideThumbnails = isset($gallery_option['hideThumbnails']) && $gallery_option['hideThumbnails'] === 'true' ? ' style="display: none"' : '';
    $hideCoverflowList = isset($gallery_option['hideCoverflowList']) && $gallery_option['hideCoverflowList'] === 'true' ? ' style="display: none"' : '';
...
function ccgallery_xml() {
    ...
    for( $i = 0; $i < $itemnum; $i++ ) {
        ...
        if( !is_wp_error($cover) ) {
            echo '<cover>'.$cover['url'].'</cover>';
        }

        if( !is_wp_error($thumb) ) {
            echo '<thumb>'.$thumb['url'].'</thumb>';
        }
...
function ccgallery_enqueue_css( $posts ) {
    ...
    if( $found ) {
		...
		$ccgallery_options = get_option( 'ccgallery_options' );
		$gallery_options = $ccgallery_options[ $id ];
		wp_enqueue_style( 'ccgallery_css', ccgallery_css_path.'/ccgallery.css', null, null );
        wp_enqueue_style( 'ccgallery_mediaelement_css', ccgallery_path.'/mediaelement/mediaelementplayer.min.css', null, null );
    }
...
// allow redirect to occur in manager page
function ccgallery_output_buffer() {
    ob_start();
}

add_action('admin_init', 'ccgallery_output_buffer');