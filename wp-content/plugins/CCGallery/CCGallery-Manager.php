 ...
 */
    ...
    if( isset( $_GET['action'] ) && $_GET['action'] === 'delete' ) {
        ...
        $new_url = remove_query_arg( array('action', 'deleted', 'cloned', '_wpnonce') );
        $new_url = 'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . $new_url;
        $new_url = add_query_arg( array( 'deleted' => true ), $new_url );
        wp_redirect( esc_url_raw( $new_url ) );
        exit;
    }
    else if( isset( $_GET['action'] ) && $_GET['action'] === 'clone' ) {
        check_admin_referer( 'ccgallery_clone_id'.$_GET['gallery_id'] );

        $gallery_options = $ccgallery_options[ $_GET['gallery_id'] ];
        $gallery_options['name'] .= ' copy';
        ...
        $new_id = $new_id + 1;
        $gallery_options['gallery_id'] = $new_id;
        $gallery_options['date'] = date('d M, Y');
        $gallery_options['shortcode'] = '[flowgallery id="'.$new_id.'"]';
        $gallery_ids[$new_id] = $new_id;
        $ccgallery_options[$new_id] = $gallery_options;
        update_option( 'ccgallery_ids', $gallery_ids );
        update_option( 'ccgallery_options', $ccgallery_options );

        $new_url = remove_query_arg( array('action', 'deleted', 'cloned', '_wpnonce') );
        $new_url = 'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . $new_url;
        $new_url = add_query_arg( array( 'cloned' => true, 'new_id' => $new_id ), $new_url );
        wp_redirect( esc_url_raw( $new_url ) );
        exit;
    }
?>
...
<div class="wrap">
    ...
    <?php
        if( isset($_GET['deleted']) ) {
            echo '<div class="updated" id="message"><p>The gallery (id = '.$_GET['gallery_id'].') was deleted</p></div>';
        }

        if( isset($_GET['cloned']) ) {
            echo '<div class="updated" id="message"><p>The gallery (id = '.$_GET['gallery_id'].') was cloned into the gallery with id = '.$_GET['new_id'].'</p></div>';
        }
    ?>
        ...
        <tbody>
            ...
            if( $gallery_ids ) {
                foreach( $gallery_ids as $gallery_id ) {
                    ...
                    $clone_query = add_query_arg( array( 'action' => 'clone', 'gallery_id' => $gallery_id ) );
                    $clone_url = wp_nonce_url( $clone_query, 'ccgallery_clone_id'.$gallery_id );
                    ...
                    <tr>
                        ...
                        <th>
                            ...
                            |
                            <a href="<?php echo esc_url($delete_url); ?>" onclick="return confirm('Are you sure that you want to delete this galleryr?');">Delete</a>
                            |
